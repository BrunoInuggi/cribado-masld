...
with col1:
    cintura = st.number_input('Cintura (cm)', min_value=0.0, max_value=200.0, value=0.0,
                              help='Riesgo: ≥94 (H) / ≥80 (M). Riesgo alto: ≥102 (H) / ≥88 (M).')
...
if modo == 'Con analítica de sangre':
    ...
    with col4:
        ...
        hdl = st.number_input('HDL colesterol (mg/dL)', min_value=0.0, max_value=150.0, value=0.0,
                              help='Deseable: >40 (H) / >50 (M) mg/dL.')
    st.subheader('Datos para FIB-4 (fibrosis) — opcional')
    col5, col6, col7 = st.columns(3)
    with col5:
        ast = st.number_input('AST/GOT (U/L)', min_value=0.0, max_value=500.0, value=0.0, help='Normal: 10-40 U/L.')
    with col6:
        alt = st.number_input('ALT/GPT (U/L)', min_value=0.0, max_value=500.0, value=0.0, help='Normal: 7-56 U/L.')
    with col7:
        plaquetas = st.number_input('Plaquetas (10⁹/L)', min_value=0.0, max_value=800.0, value=0.0, help='Normal: 150-400.')
...
if st.button('Estimar'):
    ...
    if faltan:
        ...
        # Se guardan los datos del paciente en session_state para el simulador
...
# --- Se muestran resultados si ya se estimo ---
if st.session_state.get('estimado', False):
    ...
    if modo_g == 'Solo medidas (sin analítica)':
        ...
        modelo_usado = modelo_simple if False else modelo_sangre
    ...
    fig_b.update_layout(xaxis_title='Impacto en el CAP (dB/m)', height=220,
                        margin=dict(t=10, b=40, l=10, r=10), bargap=0.15,
    ...
    st.caption('Estime cómo cambiaría el CAP con la pérdida de peso. Se asume una reducción '
               'aproximada de 0,9 cm de cintura por kg perdido (promedio poblacional). '
    ...
    def predecir_con_kg(kg):
        ...
        return modelo_usado.predict(Xx)[0], c, w

    kg_perdidos = st.slider('Kilos a perder', 0.0, 30.0, 5.0, 0.5)
    cap_sim, sim_cintura, sim_whtr = predecir_con_kg(kg_perdidos)
    delta = cap_sim - cap
    ...
    fig_s = go.Figure(go.Indicator(
        ...
        gauge={'axis': {'range': [100, 400]},
               ...
               'threshold': {'line': {'color': '#1565c0', 'width': 4},
                             'thickness': 0.8, 'value': cap}}))
    fig_s.update_layout(height=320, margin=dict(t=60, b=10, l=30, r=30),
                        paper_bgcolor='rgba(0,0,0,0)', font=dict(color='#1a1a1a'))
    st.plotly_chart(fig_s, use_container_width=True)
    st.caption(f'🔵 Línea azul = CAP actual ({cap:.0f} dB/m).  '
               f'Cintura estimada: {d["cintura"]:.0f} → {sim_cintura:.0f} cm.')

    # --- Cambio de grado ---
    grado_actual = grado_cap(cap)
    grado_nuevo = grado_cap(cap_sim)
    if kg_perdidos > 0 and grado_nuevo != grado_actual:
        ...
        st.info(f'📉 La estimación baja {abs(delta):.0f} dB/m, aunque se mantiene en grado {grado_nuevo}.')

    # --- Meta: kg para salir de riesgo (bajar de 265) o cambiar de grado ---
    if cap >= UMBRAL_CRIBADO:
        # Se busca cuantos kg harian falta para bajar del umbral de cribado
        kg_meta = None
        for kg in np.arange(0.5, 40.5, 0.5):
            cap_test, _, _ = predecir_con_kg(kg)
            if cap_test < UMBRAL_CRIBADO:
                kg_meta = kg
                break
        if kg_meta:
            st.warning(f'🏁 Meta: perdiendo aproximadamente **{kg_meta:.1f} kg**, '
                       f'saldría de la zona de riesgo de esteatosis del cribado.')
        else:
